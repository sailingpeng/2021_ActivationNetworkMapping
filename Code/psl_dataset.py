...
class Dataset(object):
    ...
    def load(cls, filename):
        ...
        if hasattr(dataset, 'feature_table'):
            dataset.feature_table.data = pd.DataFrame(dataset.feature_table.data['values'].todense(),
                                 index=dataset.feature_table.data['index'],
                                 # columns=self.data['columns']).to_sparse()
                                 columns=dataset.feature_table.data['columns']).astype(pd.SparseDtype())
...
class FeatureTable(object):
    ...
    def _csr_to_sdf(self):
        ...
        self.data = pd.DataFrame(self.data['values'].todense(),
                                 ...
                                 # columns=self.data['columns']).to_sparse()
                                 columns=self.data['columns']).astype(pd.SparseDtype())
